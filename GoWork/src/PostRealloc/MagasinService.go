...
type MagasinService struct {
	...
	resetMagasins  gorest.EndPoint `method:"DELETE" path:"/" output:"[]Magasin"`
}

func (serv MagasinService) ResetMagasins() []Magasin {
	serv.ResponseBuilder().AddHeader("Access-Control-Allow-Origin", "*")

	fmt.Printf("ResetMagasins\n")

	serv.ResponseBuilder().SetResponseCode(200)
	return resetMagasinsBouchon()
}
